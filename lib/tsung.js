...
var Tsung = module.exports.Tsung = function(config) {
    ...
    that.toXml = function(callback) {
        ...
        // build out the model to be a mustache view-model
        ...
        for (var i = 0; i < sessions.length; i++) {
            sessions[i].fileserver = fileServers;
            sessions[i].dynvarstring = dynvarStrings;
            sessions[i].dynvarnumber = dynvarNumbers;
        }
...
var Request = function(method, url, subst, data) {
    var variables = [];
    var that = {};
    that.method = method;
    that.url = url;

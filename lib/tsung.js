...
var Tsung = module.exports.Tsung = function(config) {
    ...
    var _convertModelForMustache = function() {
        ...
        for (i = 0; i < fileServers.length; i++) {
            var fileServer = fileServers[i];
            var varNames = [];
            for (j = 0; j < fileServer.vars.length; j++) {
        ...
        for (i = 0; i < sessions.length; i++) {
            ...
            for (j = 0; j < session.actions.length; j++) {
        ...
        Object.keys(transactionScore).forEach(function(txName) {
            ...
            tsungStats.transactionScore.push({'name': txName, 'score': (score / totalTransactionScore) * 100});
        });
        tsungStats.transactionScore.sort(function(a, b) { return b.score - a.score; });
    };

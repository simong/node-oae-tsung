...
// we need to artificially throttle the content and discussion access row permutations a bit, it can get crazy
var MAX_PERMUTATIONS_PER_CONTENT_DEPTH = 1;
var MAX_PERMUTATIONS_PER_DISCUSSION_DEPTH = 1;

var NUM_CONTENT_DEPTH_0 = 8;
var NUM_CONTENT_DEPTH_1 = 4;
var NUM_CONTENT_DEPTH_2 = 2;

var NUM_DISCUSSION_DEPTH_0 = 8;
var NUM_DISCUSSION_DEPTH_1 = 4;
var NUM_DISCUSSION_DEPTH_2 = 2;
 ...
 * @param {String}      csvWriter       The writer utility to write the csv file
...
module.exports = function(batchNum, model, csvWriter, callback) {
    ...
    _generatePrivateGroupAccessCsv(batchNum, model, csvWriter, monitorCsv);
    _generatePrivateContentAccessCsv(batchNum, model, csvWriter, monitorCsv);
    _generatePrivateDiscussionAccessCsv(batchNum, model, csvWriter, monitorCsv);
};
 ...
 * @param {Object}      callback.err    An error that occurred, if any
 */
var _generatePrivateGroupAccessCsv = function(batchNum, model, csvWriter, callback) {
    var rows = [];
    ...
    csvWriter.write('private_groups', rows, callback);
};
 ...
 * @param {Object}      callback.err    An error that occurred, if any
 */
var _generatePrivateContentAccessCsv = function(batchNum, model, csvWriter, callback) {
    var rows = [];
    ...
    csvWriter.write('private_content', rows, callback);
};


/**
 * Generate the CSV data that associates users to private discussions to which they have access.
 *
 * @param {Number}      batchNum        The current batch number
 * @param {Object}      model           The basic source data model
 * @param {String}      csvWriter       The writer utility to write the csv file
 ...
 * @param {Object}      callback.err    An error that occurred, if any
 */
var _generatePrivateDiscussionAccessCsv = function(batchNum, model, csvWriter, callback) {
    var rows = [];
    var discussions = model.discussions;
    var userDiscussionAccessDepth = _buildUserDiscussionAccessDepth(model);

    // build the rows
    for (var userId in userDiscussionAccessDepth) {
        var discussionAccessDepth = userDiscussionAccessDepth[userId];

        // make sure the user has enough data to be a candidate
        if (_meetsContentDepthRequirements(discussionAccessDepth)) {

            // aggregate different permutations of content and groups for the data file
            var discussion0 = [];

            // iterate over depth 0 content items
            discussionAccessDepth['0'].forEach(function(discussionId0) {
                discussion0.push(model.idMapping['discussions'][model.discussions[discussionId0].id]);
                var discussion1 = [];

                if (discussion0.length === NUM_CONTENT_DEPTH_0) {
                    var permutations0 = 0;

                    // iterate over depth 1 content items
                    discussionAccessDepth['1'].forEach(function(discussionId1) {
                        discussion1.push(model.idMapping['discussions'][model.discussions[discussionId1].id]);
                        var discussion2 = [];

                        if (discussion1.length === NUM_CONTENT_DEPTH_1 && permutations0 !== MAX_PERMUTATIONS_PER_CONTENT_DEPTH) {
                            permutations0++;

                            var permutations1 = 0;

                            // iterate over depth 2 content items
                            discussionAccessDepth['2'].forEach(function(discussionId2) {
                                discussion2.push(model.idMapping['discussions'][model.discussions[discussionId2].id]);

                                if (discussion2.length === NUM_CONTENT_DEPTH_2 && permutations1 !== MAX_PERMUTATIONS_PER_CONTENT_DEPTH) {
                                    permutations1++;

                                    // put together the user login info and content access by depth (0, 1 and 2) into one row
                                    var row = [model.users[userId].userid, model.users[userId].password];
                                    Array.prototype.push.apply(row, discussion0);
                                    Array.prototype.push.apply(row, discussion1);
                                    Array.prototype.push.apply(row, discussion2);
                                    rows.push(row);
    ...
    csvWriter.write('private_discussions', rows, callback);
};
...
var _meetsContentDepthRequirements = function(accessDepth) {
    var meetsAccessRequirements = accessDepth && accessDepth['0'] && accessDepth['1'] && accessDepth['2'] &&
            accessDepth['0'].length >= NUM_CONTENT_DEPTH_0 && accessDepth['1'].length >= NUM_CONTENT_DEPTH_1 &&
            accessDepth['2'].length >= NUM_CONTENT_DEPTH_2;

    return meetsAccessRequirements;
};

/**
 * Determine whether or not the given user discussion access depth model meets the requirements needed for it to be part of a
 * "private access" Tsung Session. This will require a certain number of groups be available at different depths, so that all
 * user sessions can be executed deterministically.
 *
 * @param   {Object}    accessDepth     The access depth model of a user for discussions. @see #_buildUserDiscussionAccessDepth
 * @return  {Boolean}                   Whether or not the discussion depth model meets the requirements
 */
var _meetsContentDepthRequirements = function(accessDepth) {
    var meetsAccessRequirements = accessDepth && accessDepth['0'] && accessDepth['1'] && accessDepth['2'] &&
            accessDepth['0'].length >= NUM_DISCUSSION_DEPTH_0 && accessDepth['1'].length >= NUM_DISCUSSION_DEPTH_1 &&
            accessDepth['2'].length >= NUM_DISCUSSION_DEPTH_2;
...
/**
 * Build a discussion access depth model that illustrates all the discussions to which a user has access, and at what level of group
 * membership depth. The result is an array of arrays of strings. The index of the outer array illustrates the group depth, while
 * the inner array holds the discussion ids to which the user has access at the current depth.
 *
 * @param   {Object}        model   The basic discussion model
 * @return  {String[][]}            The discussion access depth model as described in the summary
 */
var _buildUserDiscussionAccessDepth = function(model) {
    var discussions = model.discussions;
    var discussionAccessDepth = {};
    var userContentAccessDepth = {};

    // we only care about private discussion
    _.values(discussions).forEach(function(discussion) {
        if (discussion.visibility === 'private') {
            discussionAccessDepth[discussion.id] = _buildAccessForDiscussions(model, discussion.id, MAX_DEPTH);
        }
    });

    // invert the discussion access depth to be keyed by user
    for (var discussionId in discussionAccessDepth) {
        var access = discussionAccessDepth[discussionId];
        for (var depth = 0; depth < access.length; depth++) {
            access[depth].forEach(function(userId) {
                if (!userContentAccessDepth[userId])
                    userContentAccessDepth[userId] = {};
                if (!userContentAccessDepth[userId][depth])
                    userContentAccessDepth[userId][depth] = [];
                userContentAccessDepth[userId][depth].push(discussionId);
            });
        }
    }
...
/**
 * Build the discussion access model for a discussion. The result is an array of arrays of strings, where the index of the outer array
 * illustrates access depth, and the inner array holds the user ids that have access to the discussion at that particular depth. This
 * is similar to #_buildUserContentAccessDepth, where the difference is that this model is inverted (keyed by content, users are the
 * values). This is a necessary intermediary step to generate the former.
 *
 * @param   {Object}        model           The basic discussion model
 * @param   {String}        discussionId    The id of the discussion for which to build the model
 * @param   {Number}        maxDepth        The maximum depth we actually care about
 * @return  {String[][]}                    The access depth model as described in the summary
 */
var _buildAccessForDiscussions = function(model, discussionId, maxDepth) {
    var access = [];

    // push the direct users as the first element (depth 0)
    access.push(_allUsers(model.discussions[discussionId]));

    // push the member hierarchy for all groups
    _buildUserAccessForGroups(model, access, _allGroups(model.discussions[discussionId]), {}, 0, maxDepth + 1);
 ...
 *
 * @param   {Object}        model           The basic content or discussion model
 ...
 * @param   {Object}        visitedGroups   The groups that we've already search. This is an aggregated hashset to avoid redundant or infinite recursion
...
var _viewerGroups = function(item) {
    ...
    } else if (item && item.roles && item.roles.member) {
        return _.uniq(item.roles.member.groups);
    }
};
...
var _viewerUsers = function(item) {
    if (item && item.roles && item.roles.viewer) {
        ...
        return _.uniq(item.roles.member.users);
    }
};

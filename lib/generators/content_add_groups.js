...
module.exports = function(batchNum, model, outputDir, callback) {
    ...
    _.each(model.content, function(content) {
        ...
        while (nonViewers.length < NUM_GROUPS_PER_CONTENT) {
            ...
            if (!_.contains(_.union(content.roles.manager.groups, content.roles.viewer.groups), nonViewerGroupId)) {
                nonViewers.push(nonViewerGroupId);
            }
    ...
    var outputFile = outputDir + '/content_add_groups.csv';
    console.log('[content_add_groups.csv] Writing %s rows to %s', rows.length, outputFile);

...
/**
 * Generates the downloadable_content.csv file that provides public content that may be downloaded by anonymous or authenticated users. This file
...
module.exports = function(batchNum, model, outputDir, callback) {
    ...
    for (var id in model.content) {
        ...
        if (content.visibility === 'public' && content.contentType === 'file') {
    ...
    var outputFile = outputDir+'/downloadable_content.csv';
    console.log('[downloadable_content.csv] Writing %s rows to %s', rows.length, outputFile);

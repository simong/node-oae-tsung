    ...
    for (var id in model.content) {
        ...
        if (content.visibility === 'public') {
            if (content.contentid) {
                currContent.push(content.contentid);
                if (currContent.length === 10) {
                    rows.push(currContent);
                    currContent = [];
                }
            } else {
                console.log('Warning: Content item with no content id. Did you load the data into the system to generate the content ids first?');
            }
        }
    }

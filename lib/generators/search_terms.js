...
module.exports = function(batchNum, model, csvWriter, callback) {
    ...
    _.values(model.discussions).forEach(function(discussion) {
        _parseTerms(discussion.displayName);
        _parseTerms(discussion.username);
    });
    ...
    rows = _.keys(terms).map(function(term) {
        return [term];
    });

    csvWriter.write('search_terms_30', rows, callback);
};

...
module.exports = function(batchNum, model, outputDir, callback) {
    ...
    _.each(model.groups, function(group) {
        _.each(group.roles.manager.users, function(userId) {
            manage[userId] = manage[userId] || {'content': [], 'groups': []};
            if (!_.contains(group.roles.member.users, userId)) {
                manage[userId].groups.push(group.id);
            }
        });
    });
    ...
    _.each(manage, function(managed, userId) {
        ...
        var max = Math.max(numContent, numGroups);
        if (numContent > 0 && numGroups > 0) {
            var user = model.users[userId];
            for (var i = 0; i < max; i++) {
                // Grab different permutations of both managed users and groups
                rows.push([user.userid, user.password, managed.groups[i % numGroups], managed.content[i % numContent]]);
            }
        }
    });

    var outputFile = outputDir+'/manage_content_groups.csv';
    console.log('[manage_content_groups.csv] Writing %s rows to %s', rows.length, outputFile);

...
module.exports = function(batchNum, model, csvWriter, callback) {
    ...
    _.each(model.groups, function(group) {
        ...
        var nonMembers = [];
        while (nonMembers.length < NUM_GROUPS_PER_GROUP) {
            var groupIndex = i % groupsArray.length;
            var nonMemberGroupId = groupsArray[groupIndex].id;
            if (!_.contains(_.union(group.roles.manager.groups, group.roles.member.groups), nonMemberGroupId)) {
                nonMembers.push(model.idMapping.groups[nonMemberGroupId]);
            }

            i++;
        }

        var row = [manager.userid, manager.password, model.idMapping.groups[group.id]];
        Array.prototype.push.apply(row, nonMembers);
        rows.push(row);
    });

    csvWriter.write('group_add_groups', rows, callback);
};

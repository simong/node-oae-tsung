...
module.exports = function(batchNum, model, outputDir, callback) {
    ...
    _.each(model.groups, function(group) {
        ...
        while (nonMembers.length < NUM_GROUPS_PER_GROUP) {
            ...
            if (!_.contains(_.union(group.roles.manager.groups, group.roles.member.groups), nonMemberGroupId)) {
                nonMembers.push(nonMemberGroupId);
            }
    ...
    var outputFile = outputDir + '/group_add_groups.csv';
    console.log('[group_add_groups.csv] Writing %s rows to %s', rows.length, outputFile);

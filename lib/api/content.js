 ...
 *
 * @param  {Session}    session     A session
 * @param  {String}     displayName The name of a dynamic variable that represents a link displayName.
 ...
 *                                  of this link.
 */
var createLink = module.exports.createLink = function(session, displayName, description, visibility, link, managers, viewers) {
 ...
 */
var update = module.exports.update = function(session, contentId, fields) {
    var tx = session.addTransaction('content_update');
    tx.addRequest('POST', '/api/content/' + contentId, fields);
};
...
var share = module.exports.share = function(session, contentId, shareWith) {
    ...
    tx.addRequest('POST', '/api/content/' + contentId + '/share', {'viewers': shareWith});
};
 ...
 *
 * @param  {Session}    session     A session
 * @param  {String}     contentId   A variable that represents the content id
 * @param  {Object}     updates     An object whose key is a variable representing a principal and value is one of 'manager', 'viewer', or `false`, indicating how to update their membership with the content item. `false` indicates that the principal should be removed.
 */
var updateMembers = module.exports.updateMembers = function(session, contentId, updates) {
    var tx = session.addTransaction('content_update_members');
    tx.addRequest('POST', '/api/content/' + contentId + '/members', updates);
};

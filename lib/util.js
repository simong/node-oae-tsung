...
var normalizeProbabilities = function(testCases) {
    ...
    Object.keys(testCases).forEach(function(name) {
        ...
        totalWeight += parseInt(testCases[name], 10);
    });
    ...
    if (totalWeight < 100) {
        var key = Object.keys(testCases)[0];
        testCases[key] = parseInt(testCases[key], 10) + (100 - totalWeight);
    }

    // If we have a weight that evaluates to 0, do not continue. The user must adjust the weights to either give value or remove it
    var zeroWeights = [];
    Object.keys(testCases).forEach(function(name) {
        if (testCases[name] === 0) {
            zeroWeights.push(name);
        }
    });

    if (zeroWeights.length > 0) {
        throw new Error('Test weights result in a weight of 0 for test cases: ' + zeroWeights.join(', ') + '. Increase their test probability, or remove them.');
    }
};
